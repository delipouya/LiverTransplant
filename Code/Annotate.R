...
merged_samples = readRDS('Objects/Integrated_Syngeneic_mt15_lib1500_harmony.rds')
merged_samples = readRDS('Objects/Integrated_Syngeneic_mt5_lib1500_harmony.rds')
...
ggplot(df_umap, aes(x=UMAP_1, y=UMAP_2, color=label))+geom_point(size=3,alpha=1)+
  theme_classic()+scale_color_manual(values = c(colorPalatte, 'black')) #mycolors

df_umap$label_set2 = df_umap$label  
###### generating count barplots

df <- data.frame(sample_type = merged_samples$ID, 
                 cluster = as.character(df_umap$annotation_g))
rownames(df) = NULL
